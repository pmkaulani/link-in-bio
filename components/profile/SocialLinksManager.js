...
export default function SocialLinksManager({ profile, updateProfile, isLinksPage = false }) {
  ...
  // On the Links page, when collapsed, show a sleek single-row card
  if (isLinksPage && isSectionCollapsed) {
    return (
      <div className="py-2">
        <div className="flex items-center justify-between rounded-[12px] border border-zinc-200 bg-white p-3 shadow-xs hover:border-zinc-300 transition">
          <div
            onClick={() => setIsSectionCollapsed(false)}
            className="flex items-center gap-2.5 min-w-0 flex-1 cursor-pointer select-none"
            title="Click to expand social accounts"
          >
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[6px] bg-zinc-100 text-black">
              <Share2 size={15} />
            </div>
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-xs font-bold text-black whitespace-nowrap">Social Accounts</span>
                <span className="rounded-full bg-zinc-100 border border-zinc-200 px-2 py-0.2 text-[10px] font-bold text-zinc-600">
                  {localAccounts.length}
                </span>
                {!isGlobalVisible && (
                  <span className="flex items-center gap-1 rounded-full bg-amber-50 border border-amber-200 px-1.5 py-0.2 text-[9px] font-bold text-amber-700 whitespace-nowrap">
                    <EyeOff size={9} /> Hidden
                  </span>
                )}
              </div>
              <span className="block truncate text-[11px] text-zinc-400 font-medium mt-0.5">
                {localAccounts.length > 0
                  ? `${localAccounts.map((a) => PLATFORMS[a.platform]?.label || a.platform).slice(0, 4).join(', ')}${localAccounts.length > 4 ? '...' : ''}`
          ...
          <div className="flex items-center gap-1.5 shrink-0 ml-2">
            <button
              ...
              className={`rounded-full p-1.5 transition ${
                isGlobalVisible ? 'text-zinc-400 hover:bg-zinc-100 hover:text-black' : 'text-amber-600 bg-amber-50 hover:bg-amber-100'
              }`}
              title={isGlobalVisible ? 'Visible on profile (click to hide)' : 'Hidden on profile (click to show)'}
            >
              {isGlobalVisible ? <Eye size={15} /> : <EyeOff size={15} />}
            </button>
            <button
              type="button"
              onClick={() => setIsSectionCollapsed(false)}
              className="flex items-center gap-1 rounded-[8px] bg-zinc-100 hover:bg-zinc-200 px-2.5 py-1 text-xs font-bold text-zinc-700 transition"
            >
              <span>Manage</span>
              <ChevronDown size={13} />
            </button>
  ...
  return (
    <div className="py-5 border-t border-zinc-200">
      ...
      <div className="flex flex-col gap-2.5 mb-4">
        ...
        {/* Actions Bar: Visibility Toggle & Add Button */}
      ...
      {/* Accounts List (Single column stack matching SortableBlock style) */}
      {localAccounts.length > 0 ? (
        <div className="flex flex-col gap-2.5">
          {localAccounts.map((acc) => {
            ...
            return (
              <div
                key={acc.id}
                className={`group flex items-center justify-between gap-3 rounded-[12px] border bg-white px-3.5 py-3 shadow-xs hover:border-zinc-300 transition animate-profile-in ${
                  isAccVisible ? 'border-zinc-200' : 'border-zinc-200/60 bg-zinc-50/60 opacity-75'
                }`}
              >
                {/* Left: Platform Icon & Info */}
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[8px] bg-zinc-100 text-black border border-zinc-200/80 shadow-xs">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  {samePlatformCount > 1 && !acc.is_primary && (
                    <button
                      ...
                      className="flex items-center gap-1 rounded-[6px] border border-zinc-200 px-2 py-1 text-[10px] font-bold text-zinc-600 hover:border-black hover:text-black transition mr-1"
                  ...
                  >
                    {isAccVisible ? <Eye size={15} /> : <EyeOff size={15} />}
                  </button>
