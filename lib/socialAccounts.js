...
export function normalizeSocialAccounts(profile = {}) {
  ...
  if (profile.socials && Array.isArray(profile.socials._accounts) && profile.socials._accounts.length > 0) {
...
export function syncSocialsWithAccounts(accounts = [], existingSocials = {}) {
  ...
  // Preserve internal metadata keys (like _password_set, _active_sessions, etc.)
  ...
  // For each platform, assign the primary account's URL to socials[platform]
  ...
  for (const [platform, list] of Object.entries(platformGroups)) {
    ...
    if (primary && primary.url) {
      cleanSocials[platform] = primary.url;
    }
  }
 ...
 */
export function groupAccountsByPlatform(accounts = []) {
  const groups = {};
