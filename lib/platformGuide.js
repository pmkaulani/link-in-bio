...
export const COUNTRY_CODES_BY_LENGTH = [...COUNTRY_CODES].sort(
  (a, b) => b.code.replace(/\D/g, '').length - a.code.replace(/\D/g, '').length
);

/**
 * Robust phone number splitter that extracts country code and national number
 * without duplicating prefixes or breaking local numbers.
 */
export function splitPhoneNumber(input, defaultCountryCode = '+1') {
  if (!input || typeof input !== 'string') {
    return { countryCode: defaultCountryCode, nationalNumber: '', fullDigits: '' };
  }
  ...
  const defCode = defaultCountryCode.startsWith('+') ? defaultCountryCode : `+${defaultCountryCode}`;
  const defDigits = defCode.replace(/\D/g, '');

  // 1. Extract from known URL patterns
  const waMatch = raw.match(/(?:wa\.me|whatsapp\.com\/send\?phone=)\/?([+0-9]+)/i);
  const isUrl = Boolean(waMatch || /^tel:/i.test(raw));
  if (waMatch) {
    raw = waMatch[1];
  ...
  // 2. If it starts with '+', match against country codes sorted by digit length descending
  if (raw.startsWith('+')) {
    const rawNoPlus = raw.slice(1);
    const digitsOnly = rawNoPlus.replace(/\D/g, '');
    for (const c of COUNTRY_CODES_BY_LENGTH) {
      const cDigits = c.code.replace(/\D/g, '');
      if (digitsOnly.startsWith(cDigits)) {
        const national = digitsOnly.slice(cDigits.length);
        return {
          countryCode: c.code,
          nationalNumber: national,
          fullDigits: `${cDigits}${national}`,
        };
      }
    }
    const plusMatch = raw.match(/^\+(\d{1,4})(.*)$/);
    if (plusMatch) {
      const national = plusMatch[2].trim().replace(/^[-.\s]+/, '').replace(/\D/g, '');
      return {
        countryCode: `+${plusMatch[1]}`,
        nationalNumber: national,
        fullDigits: `${plusMatch[1]}${national}`,
      };
    }
  }
  ...
  // 3. If source was a full URL, match against all country codes
  if (isUrl) {
    for (const c of COUNTRY_CODES_BY_LENGTH) {
      const cDigits = c.code.replace(/\D/g, '');
      if (digits.startsWith(cDigits) && digits.length > cDigits.length) {
        const national = digits.slice(cDigits.length);
        return {
          countryCode: c.code,
          nationalNumber: national,
          fullDigits: digits,
  ...
  // 4. If digits start with the defaultCountryCode digits and have equal or more digits following
  if (defDigits && digits.startsWith(defDigits) && digits.length >= defDigits.length) {
    const national = digits.slice(defDigits.length);
    return {
      countryCode: defCode,
      nationalNumber: national,
      fullDigits: digits,
  ...
  // 5. Default: treat raw as national number for the selected country code
  return {
    countryCode: defCode,
    nationalNumber: raw,
    fullDigits: `${defDigits}${digits.replace(/^0+/, '')}`,
  };
}
...
export const PLATFORMS = {
  ...
  whatsapp: {
    ...
    buildUrl: (phone, countryCode = '+1') => {
      ...
      const { countryCode: code, nationalNumber } = splitPhoneNumber(trimmed, countryCode);
      const codeDigits = code.replace(/\D/g, '');
      const cleanNational = nationalNumber.replace(/^0+/, '').replace(/\D/g, '');
      if (!cleanNational) return '';
      return `https://wa.me/${codeDigits}${cleanNational}`;
    },
    parseUrl: (url) => {
  ...
  facebook: {
    ...
    mode: 'username',
    prefix: 'facebook.com/',
    placeholder: 'username_or_page',
    example: 'zuck',
    tip: 'Enter your Facebook username or page handle.',
    defaultTitle: (username) => (username ? `${username} on Facebook` : 'Facebook'),
    defaultSubtitle: 'Connect on Facebook',
    buildUrl: (username) => `https://facebook.com/${cleanUsername(username)}`,
    parseUrl: (url) => {
  ...
  paypal: {
    ...
    placeholder: 'username',
    example: 'yourname',
    tip: 'Enter your PayPal.me username.',
    defaultTitle: () => 'Pay with PayPal',
    defaultSubtitle: 'Support & tips',
    buildUrl: (username) => `https://paypal.me/${cleanUsername(username)}`,
    parseUrl: (url) => {
      const match = url.match(/paypal\.me\/([A-Za-z0-9_.-]+)/i);
      return match ? match[1].replace(/\/$/, '') : null;
    },
  },
  cashapp: {
    ...
    placeholder: 'cashtag',
    example: 'cashtag',
    tip: 'Enter your Cash App $cashtag without the $ symbol.',
    defaultTitle: () => 'Support on Cash App',
    defaultSubtitle: 'Tips & donations',
    buildUrl: (username) => `https://cash.app/$${cleanUsername(username).replace(/^\$/, '')}`,
    parseUrl: (url) => {
      const match = url.match(/cash\.app\/\$?([A-Za-z0-9_]+)/i);
      return match ? match[1].replace(/\/$/, '') : null;
    },
  },
  venmo: {
    ...
    placeholder: 'username',
    example: 'username',
    tip: 'Enter your Venmo handle without the @ symbol.',
    defaultTitle: () => 'Pay via Venmo',
    defaultSubtitle: 'Tips & payments',
    buildUrl: (username) => `https://venmo.com/u/${cleanUsername(username).replace(/^@/, '')}`,
    parseUrl: (url) => {
      const match = url.match(/venmo\.com\/(?:u\/)?([A-Za-z0-9_.-]+)/i);
      return match ? match[1].replace(/\/$/, '') : null;
    },
  },
  email: {
  ...
  phone: {
    ...
    buildUrl: (phone, countryCode = '+1') => {
      ...
      const cleanNational = nationalNumber.replace(/^0+/, '').replace(/\D/g, '');
      if (!cleanNational) return '';
      return `tel:+${codeDigits}${cleanNational}`;
    },
    parseUrl: (url) => {
